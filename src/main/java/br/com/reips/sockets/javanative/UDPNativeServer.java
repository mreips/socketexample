	...
	public void run() {
		try {
			...
			while (!server.isClosed()) {
				...
				final SimpleClient client = getClients().getOrDefault(address.getHostAddress(), new SimpleClient(address.getHostName(), packet.getPort()));
				getClients().putIfAbsent(address.getHostAddress(), client);
